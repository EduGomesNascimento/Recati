...
const el = {
  ...
  caixaItemForm: document.getElementById("caixa-item-form"),
  caixaItemProduto: document.getElementById("caixa-item-produto"),
  caixaItemQtd: document.getElementById("caixa-item-qtd"),
  caixaItemDesconto: document.getElementById("caixa-item-desconto"),
  caixaItemObs: document.getElementById("caixa-item-obs"),
  caixaCatalogoSearch: document.getElementById("caixa-catalogo-search"),
  caixaCatalogoList: document.getElementById("caixa-catalogo-list"),
  caixaItensList: document.getElementById("caixa-itens-list"),
...
async function api(path, options = {}) {
  if (!window.ChurrascariaMockApi || typeof window.ChurrascariaMockApi.request !== "function") {
    throw new Error("Mock API indisponivel.");
  }
  return window.ChurrascariaMockApi.request(path, options);
}
...
function produtoTemControleEstoque(produto) {
  return Boolean(produto && produto.controla_estoque !== false);
}

function toggleProdutoEstoqueInput() {
  if (!el.produtoEstoque || !el.produtoControlaEstoque) return;
  const controla = el.produtoControlaEstoque.checked;
  el.produtoEstoque.disabled = !controla;
  if (!controla) {
...
function normalizeERPConfig(payload = {}) {
  ...
  next.permitir_status_pronto = false;
  next.finalizar_mobile_status =
    next.finalizar_mobile_status === "FINALIZADA" ? "FINALIZADA" : "ABERTO";
...
function fillConfigForm(config) {
  ...
  if (el.cfgImpressaoCozinhaAutomatica) {
...
function buildPixCode() {
  ...
  const chave = (el.pixChave && el.pixChave.value ? el.pixChave.value : "churrascaria@erp.local").trim();
...
function setActiveTab(tab, persist = true) {
  ...
  if (persist) {
    localStorage.setItem("churrascaria_caixa_tab", nextTab);
  }
}
...
function loadActiveTab() {
  const saved = localStorage.getItem("churrascaria_caixa_tab") || "comandas";
  setActiveTab(saved, false);
}

async function irParaPagamentoDaComandaSelecionada() {
  if (!state.selecionada) {
  ...
  if (state.selecionada.status === "ABERTO") {
    await api(`/comandas/${state.selecionada.id}/status`, {
      method: "PATCH",
      body: JSON.stringify({ status: "FINALIZADA", repor_estoque: true }),
    });
  ...
  showToast(`Comanda ${state.selecionada.comanda_codigo} pronta para pagamento.`);
}
  ...
  state.adicionais.forEach((adicional) => {
    ...
    row.innerHTML = `
      <div class="row-head">
        ...
        <span class="${badgeClass(adicional.ativo ? "ABERTO" : "CANCELADO")}">
...
function renderProdutos() {
  ...
  state.produtos.forEach((produto) => {
    ...
    const controlaEstoque = produtoTemControleEstoque(produto);
    const row = document.createElement("div");
    ...
    row.innerHTML = `
      <div class="produto-head">
        ...
        <span class="${badgeClass(produto.ativo ? "ABERTO" : "CANCELADO")}">
      ...
      <div class="produto-controls">
        <span class="produto-stock">
          ${controlaEstoque ? `Estoque: <strong>${produto.estoque_atual}</strong>` : "<strong>Sem controle de estoque</strong>"}
        </span>
        <div class="payment-actions">
          <button type="button" data-produto-edit="${produto.id}">Editar</button>
          <button type="button" data-produto-delta="${produto.id}" data-delta="-10" ${controlaEstoque ? "" : "disabled"}>-10</button>
          <button type="button" data-produto-delta="${produto.id}" data-delta="-1" ${controlaEstoque ? "" : "disabled"}>-1</button>
          <button type="button" data-produto-delta="${produto.id}" data-delta="1" ${controlaEstoque ? "" : "disabled"}>+1</button>
          <button type="button" data-produto-delta="${produto.id}" data-delta="10" ${controlaEstoque ? "" : "disabled"}>+10</button>
...
function renderComandasResumo() {
  ...
  const finalizadas = state.comandas.filter((c) => c.status === "FINALIZADA").length;
  ...
  el.comandaResumo.innerHTML = `
    ...
    <span class="resumo-chip">Finalizadas: ${finalizadas}</span>
...
function renderAjustesItensPagamento() {
  ...
  const bloqueado = ["CANCELADO"].includes(state.selecionada.status);
  itens.forEach((item) => {
    const row = document.createElement("div");
    row.className = "row";
    row.innerHTML = `
...
function statusPermiteEditarItensNoCaixa(status) {
  return ["ABERTO", "FINALIZADA"].includes(String(status || "").toUpperCase());
}

function toggleFormularioItensCaixa(disabled) {
  const controls = [
    el.caixaItemProduto,
    el.caixaItemQtd,
    el.caixaItemDesconto,
    el.caixaItemObs,
    el.caixaCatalogoSearch,
    el.caixaItemForm ? el.caixaItemForm.querySelector("button[type='submit']") : null,
  ].filter(Boolean);
  controls.forEach((control) => {
    control.disabled = Boolean(disabled);
  });
  if (el.caixaCatalogoList) {
    el.caixaCatalogoList
      .querySelectorAll("button[data-caixa-catalog-add]")
      .forEach((button) => {
...
function renderCaixaItemProdutoSelect() {
  if (!el.caixaItemProduto) return;
  const selectedAtual = Number(el.caixaItemProduto.value || "0");
  const ativos = (state.produtos || []).filter((row) => row.ativo !== false);
  const sorted = [...ativos].sort((a, b) => String(a.nome || "").localeCompare(String(b.nome || "")));
  el.caixaItemProduto.innerHTML = "";
  if (!sorted.length) {
    const option = document.createElement("option");
    option.value = "";
    option.textContent = "Sem produtos ativos";
    option.disabled = true;
    option.selected = true;
    el.caixaItemProduto.appendChild(option);
    return;
  }
  sorted.forEach((produto) => {
    const option = document.createElement("option");
    option.value = String(produto.id);
    option.textContent = `${produto.nome} - R$ ${fmtMoney(produto.preco)}`;
    option.selected = produto.id === selectedAtual;
    el.caixaItemProduto.appendChild(option);
  });
  if (!sorted.some((row) => row.id === selectedAtual)) {
    el.caixaItemProduto.value = String(sorted[0].id);
  }
}

function renderCaixaCatalogo() {
  if (!el.caixaCatalogoList) return;
  el.caixaCatalogoList.innerHTML = "";

  const ativos = (state.produtos || []).filter((row) => row.ativo !== false);
  const termo = normalizeHintText(el.caixaCatalogoSearch ? el.caixaCatalogoSearch.value : "");
  if (!ativos.length) {
    const empty = document.createElement("p");
    empty.className = "small";
    empty.textContent = "Sem produtos ativos para o catalogo.";
    el.caixaCatalogoList.appendChild(empty);
    return;
  }
  ...
  const docinhoHints = ["doc", "doce", "sobremesa", "brigadeiro", "beijinho", "trufa"];
  const docinhos = [];
  const mobile = [];
  const outros = [];
  ...
  ativos.forEach((produto) => {
    ...
    const textoBusca = `${nome} ${categoria}`.toLowerCase();
    if (termo && !textoBusca.includes(termo)) return;
    if (mobileIds.has(produto.id)) {
      mobile.push(produto);
      return;
    }
    if (matchHint(nome, docinhoHints) || matchHint(categoria, docinhoHints)) {
      docinhos.push(produto);
      return;
    }
    outros.push(produto);
  });
  ...
  const grupos = [
    { titulo: "Itens do mobile", itens: mobile },
    { titulo: "Docinhos do caixa", itens: docinhos },
    { titulo: "Outros itens", itens: outros },
  ];
  let totalRenderizado = 0;
  grupos.forEach((grupo) => {
    if (!grupo.itens.length) return;
    totalRenderizado += grupo.itens.length;
    const box = document.createElement("section");
    box.className = "caixa-catalogo-group";
    const titulo = document.createElement("h4");
    titulo.textContent = grupo.titulo;
    box.appendChild(titulo);
    const grid = document.createElement("div");
    grid.className = "caixa-catalogo-grid";
    grupo.itens
      .sort((a, b) => String(a.nome || "").localeCompare(String(b.nome || "")))
      .forEach((produto) => {
        const item = document.createElement("div");
        item.className = "caixa-catalogo-item";
        item.innerHTML = `
          <span>${produto.nome} | R$ ${fmtMoney(produto.preco)}</span>
          <button type="button" data-caixa-catalog-add="${produto.id}">Adicionar</button>
        `;
        grid.appendChild(item);
      });
    box.appendChild(grid);
    el.caixaCatalogoList.appendChild(box);
  });

  if (!totalRenderizado) {
    const empty = document.createElement("p");
    empty.className = "small";
    empty.textContent = "Nenhum item encontrado no catalogo com esse filtro.";
    el.caixaCatalogoList.appendChild(empty);
  }
}

function renderCaixaItens() {
  if (!el.caixaItensList) return;
  el.caixaItensList.innerHTML = "";
  renderCaixaItemProdutoSelect();
  renderCaixaCatalogo();

  if (!state.selecionada) {
    toggleFormularioItensCaixa(true);
    const p = document.createElement("p");
    p.className = "small";
    p.textContent = "Selecione uma comanda para adicionar itens no caixa.";
    el.caixaItensList.appendChild(p);
    return;
  }

  const editavel = statusPermiteEditarItensNoCaixa(state.selecionada.status);
  toggleFormularioItensCaixa(!editavel);
  if (!editavel) {
    const aviso = document.createElement("p");
    aviso.className = "small";
    aviso.textContent = "Comanda cancelada nao aceita novos itens no caixa.";
    el.caixaItensList.appendChild(aviso);
  }
  ...
  if (!itens.length) {
    const p = document.createElement("p");
    p.className = "small";
    p.textContent = "Sem itens nessa comanda.";
    el.caixaItensList.appendChild(p);
    return;
  }
  ...
  itens.forEach((item) => {
    const row = document.createElement("div");
    row.className = "row";
    const observacao = item.observacoes ? `Obs: ${item.observacoes}` : "Sem observacao";
    row.innerHTML = `
      <div class="row-head">
        <strong>${item.produto_nome} x${item.quantidade}</strong>
        <span>R$ ${fmtMoney(item.subtotal)}</span>
      </div>
      <div class="small">${observacao}</div>
      <div class="payment-actions">
        <button type="button" class="btn-no" data-caixa-remove-item="${item.id}" ${editavel ? "" : "disabled"}>
          Remover item
        </button>
      </div>
    `;
    el.caixaItensList.appendChild(row);
  });
}
...
function resetProdutoForm() {
  ...
  if (el.produtoControlaEstoque) {
    el.produtoControlaEstoque.checked = true;
  }
...
function montarPayloadProduto(imagemUrlFinal = null) {
  ...
  const controlaEstoque = el.produtoControlaEstoque ? el.produtoControlaEstoque.checked : true;
  ...
  return {
    ...
    estoque_atual: controlaEstoque ? estoque : 0,
...
function preencherFormProdutoParaEdicao(produtoId) {
  ...
  if (el.produtoControlaEstoque) {
    el.produtoControlaEstoque.checked = produto.controla_estoque !== false;
  }
...
async function excluirProdutoDefinitivo(produtoId) {
  ...
  const confirmou = window.confirm(
    `Excluir '${produto.nome}'? Se ja houver historico, ele sera desativado automaticamente.`
  );
  if (!confirmou) return;

  let excluidoDefinitivo = false;
  try {
    await api(`/produtos/${produtoId}?hard=true`, { method: "DELETE" });
    excluidoDefinitivo = true;
  } catch (error) {
    const detail = String(error && error.message ? error.message : "");
    const usadoEmHistorico =
      detail.toLowerCase().includes("utilizado em pedidos/comandas") ||
      detail.toLowerCase().includes("n?o pode ser exclu?do") ||
      detail.toLowerCase().includes("nao pode ser excluido");
    if (!usadoEmHistorico) {
      throw error;
    }
    await alternarProdutoAtivo(produtoId, false);
    return;
  }
  ...
  await loadProdutos();
  showToast(
    excluidoDefinitivo ? "Produto excluido definitivamente." : "Produto desativado."
  );
}
...
async function adicionarItemNoCaixa(event) {
  event.preventDefault();
  const produtoId = Number(el.caixaItemProduto ? el.caixaItemProduto.value : "0");
  const quantidade = Number.parseInt(el.caixaItemQtd ? el.caixaItemQtd.value : "0", 10);
  const desconto = toMoneyNumber(el.caixaItemDesconto ? el.caixaItemDesconto.value : "0");
  const observacoes = el.caixaItemObs ? el.caixaItemObs.value.trim() : "";
  await adicionarItemNoCaixaPorPayload({
    produtoId,
    quantidade,
    desconto,
    observacoes,
  });
  if (el.caixaItemQtd) el.caixaItemQtd.value = "1";
  if (el.caixaItemDesconto) el.caixaItemDesconto.value = "0.00";
  if (el.caixaItemObs) el.caixaItemObs.value = "";
}

async function adicionarItemNoCaixaPorPayload({
  produtoId,
  quantidade = 1,
  desconto = 0,
  observacoes = "",
  ...
  if (!state.selecionada) throw new Error("Selecione uma comanda.");
  if (!statusPermiteEditarItensNoCaixa(state.selecionada.status)) {
    throw new Error("Status atual nao permite adicionar item no caixa.");
  }
  if (!produtoId) throw new Error("Selecione um produto para adicionar.");
  if (!Number.isInteger(quantidade) || quantidade <= 0) {
    throw new Error("Quantidade invalida.");
  }
  if (desconto < 0) throw new Error("Desconto invalido.");
  ...
  await selecionarComanda(state.selecionada.id);
  await loadComandas();
  showToast("Item adicionado no caixa.");
}

async function removerItemNoCaixa(itemId) {
  if (!state.selecionada) throw new Error("Selecione uma comanda.");
  if (!statusPermiteEditarItensNoCaixa(state.selecionada.status)) {
    throw new Error("Status atual nao permite remover item no caixa.");
  }
  const item = (state.selecionada.itens || []).find((row) => row.id === itemId);
  if (!item) throw new Error("Item nao encontrado na comanda.");

  await api(`/comandas/${state.selecionada.id}/itens/${itemId}`, { method: "DELETE" });
  await selecionarComanda(state.selecionada.id);
  await loadComandas();
  showToast("Item removido do caixa.");
}
...
function applyUIOptions(options, { persist = true, syncForm = true } = {}) {
  ...
  if (persist) {
    localStorage.setItem("churrascaria_caixa_ui_options", JSON.stringify(next));
  }
  if (syncForm) {
...
function loadUIOptions() {
  let parsed = null;
  try {
    const raw = localStorage.getItem("churrascaria_caixa_ui_options");
...
function bind() {
  ...
  on(el.produtoControlaEstoque, "change", () => toggleProdutoEstoqueInput());
  ...
  on(el.goPagamento, "click", () => run(() => irParaPagamentoDaComandaSelecionada()));
  on(el.deleteComanda, "click", () => run(() => excluirComandaSelecionada()));
  on(el.caixaItemForm, "submit", (event) => run(() => adicionarItemNoCaixa(event)));
  on(el.caixaCatalogoSearch, "input", renderCaixaCatalogo);
  on(el.caixaCatalogoList, "click", (event) => {
    const addBtn = event.target.closest("button[data-caixa-catalog-add]");
    if (!addBtn) return;
    const produtoId = Number(addBtn.dataset.caixaCatalogAdd || "0");
    run(() => adicionarItemNoCaixaPorPayload({ produtoId, quantidade: 1, desconto: 0, observacoes: null }));
  });
  on(el.caixaItensList, "click", (event) => {
    const removeBtn = event.target.closest("button[data-caixa-remove-item]");
    if (!removeBtn) return;
    const itemId = Number(removeBtn.dataset.caixaRemoveItem || "0");
    run(() => removerItemNoCaixa(itemId));
  });
