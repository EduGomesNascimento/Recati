...
const el = {
  ...
  finalizarPedidoBtn: document.getElementById("finalizar-pedido"),
...
const STATUS_EDITAVEIS_ITENS = new Set(["ABERTO"]);
const STATUS_PAINEL_EDITAVEL = new Set(["ABERTO"]);
...
function applyUIOptions(options, { persist = true, syncForm = true } = {}) {
  ...
  if (persist) {
    localStorage.setItem("churrascaria_mobile_ui_options", JSON.stringify(next));
  }
  if (syncForm) {
...
function loadUIOptions() {
  let parsed = null;
  try {
    const raw = localStorage.getItem("churrascaria_mobile_ui_options");
...
function setActiveTab(tab, persist = true) {
  ...
  if (persist) {
    localStorage.setItem("churrascaria_mobile_tab", nextTab);
  }
}

function loadActiveTab() {
  const saved = localStorage.getItem("churrascaria_mobile_tab") || "pedidos";
...
function renderPainelComandas() {
  ...
  rows.forEach((row) => {
    ...
    if (STATUS_PAINEL_EDITAVEL.has(row.status || "")) {
      acao = "Toque para editar";
    } else if (row.status === "FINALIZADA" || row.status === "CANCELADO") {
...
async function garantirCodigosPadraoSeNecessario() {
  const codigos = await api("/comandas/codigos?ativo=true");
  if (Array.isArray(codigos) && codigos.length) {
    return false;
  }
  for (let numero = 1; numero <= 20; numero += 1) {
    const codigo = `C-${String(numero).padStart(3, "0")}`;
    try {
      ...
      const detalhe = String(error && error.message ? error.message : "");
      if (!detalhe.toLowerCase().includes("já cadastrado")) {
        throw error;
      }
    }
  }
...
async function carregarPainelComandas() {
  ...
  if (!state.painelComandas.length) {
    const criouPadrao = await garantirCodigosPadraoSeNecessario();
    if (criouPadrao) {
      state.painelComandas = await api("/comandas/painel?ativo=true");
    }
  }
...
async function agirComandaPainel(codigoId, status, pedidoId) {
  ...
  if (status === "FINALIZADA" || status === "CANCELADO") {
...
async function abrirComanda(event) {
  ...
  const created = await api("/comandas/abrir", {
    ...
    body: JSON.stringify({
      codigo,
      tipo_entrega: el.tipoEntrega.value,
      mesa: sanitizeOptionalText(el.mesaComanda.value),
...
async function mudarStatus(status) {
  ...
  if (status === "FINALIZADA") {
    const confirma = window.confirm("Finalizar o pedido da comanda selecionada?");
    if (!confirma) return;
    reporEstoque = false;
  }
  if (status === "CANCELADO") {
...
function bind() {
  ...
  if (el.finalizarPedidoBtn) {
    el.finalizarPedidoBtn.addEventListener("click", () => run(() => mudarStatus("FINALIZADA")));
  }
