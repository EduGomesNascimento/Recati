(() => {
  ...
  const views = [["institucional","Institucional"],["servicos","Serviços"],["cases","Cases"],["painel","Painel"]];
  ...
  function renderPainel(){
    ...
    rows.innerHTML = state.logs
      .slice()
      .reverse()
      .map((l) => `<tr><td>${new Date(l.data).toLocaleString("pt-BR")}</td><td>${l.modulo}</td><td>${l.evento}</td></tr>`)
      .join("");
    ...
    document.getElementById("exportCsv").addEventListener("click", () => {
      ...
      const lines = [header.join(",")].concat(
        state.logs.map((l) => [l.data, l.modulo, l.evento].map((x) => `"${String(x).replaceAll('"', '""')}"`).join(","))
      );
