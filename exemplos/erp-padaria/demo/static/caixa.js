...
const CUSTOM_TEXT_TARGETS = Object.freeze({
  top_subtitulo: "brand-app-subtitle",
  aba_comandas: "txt-tab-comandas",
  aba_pagamentos: "txt-tab-pagamentos",
  aba_produtos_servicos: "txt-tab-produtos",
  aba_opcoes: "txt-tab-opcoes",
  botao_abrir_mobile: "go-mobile-link",
  titulo_codigos_comanda: "txt-summary-codigos",
  titulo_comandas: "txt-summary-comandas",
  titulo_comanda_selecionada: "txt-summary-selecionada",
  titulo_produtos: "txt-summary-produtos",
  titulo_adicionais: "txt-summary-adicionais",
  titulo_opcoes: "txt-summary-opcoes",
});

const CUSTOM_TEXT_DEFAULTS = Object.freeze({
  top_subtitulo: "Caixa, Pagamento e Fechamento",
  aba_comandas: "Comandas",
  aba_pagamentos: "Pagamentos",
  aba_produtos_servicos: "Produtos e Serviços",
  aba_opcoes: "Opções",
  botao_abrir_mobile: "Abrir App Mobile",
  titulo_codigos_comanda: "Códigos de Comanda",
  titulo_comandas: "Comandas",
  titulo_comanda_selecionada: "Comanda Selecionada",
  titulo_produtos: "Produtos",
  titulo_adicionais: "Adicionais",
  titulo_opcoes: "Parâmetros Customizáveis",
});
...
const el = {
  ...
  produtoControlaEstoque: document.getElementById("produto-controla-estoque"),
  ...
  caixaItemSearch: document.getElementById("caixa-item-search"),
  caixaMobileAtalhos: document.getElementById("caixa-mobile-atalhos"),
  caixaCatalogoItens: document.getElementById("caixa-catalogo-itens"),
  caixaComandaItensList: document.getElementById("caixa-comanda-itens-list"),
  ...
  cfgLayoutCss: document.getElementById("cfg-layout-css"),
  cfgLayoutTextos: document.getElementById("cfg-layout-textos"),
...
const COMANDA_ITEM_EDIT_STATUSES = new Set(["ABERTO", "EM_PREPARO", "PRONTO", "ENTREGUE"]);
...
async function api(path, options = {}) {
  if (!window.PadariaMockApi || typeof window.PadariaMockApi.request !== "function") {
    throw new Error("Mock API indisponivel.");
  }
  return window.PadariaMockApi.request(path, options);
}
...
function normalizeLayoutTextMap(value) {
  const source = value && typeof value === "object" && !Array.isArray(value) ? value : {};
  const clean = {};
  Object.keys(CUSTOM_TEXT_TARGETS).forEach((key) => {
    const raw = source[key];
    if (raw === undefined || raw === null) return;
    const text = String(raw).trim().slice(0, 220);
    if (!text) return;
    clean[key] = text;
  });
  return clean;
...
function buildDefaultCustomTexts(config) {
  return {
    ...CUSTOM_TEXT_DEFAULTS,
    top_subtitulo: config.empresa_subtitulo,
  };
}

function applyCustomLayout(config) {
  const cssId = "custom-layout-css";
  let styleNode = document.getElementById(cssId);
  if (!styleNode) {
    styleNode = document.createElement("style");
    styleNode.id = cssId;
    document.head.appendChild(styleNode);
  }
  styleNode.textContent = String(config.layout_css_custom || "");

  const defaults = buildDefaultCustomTexts(config);
  const customMap = normalizeLayoutTextMap(config.layout_textos_custom);
  Object.entries(CUSTOM_TEXT_TARGETS).forEach(([key, elementId]) => {
    const node = document.getElementById(elementId);
    if (!node) return;
    const text = customMap[key] || defaults[key] || "";
...
function normalizeERPConfig(payload = {}) {
  ...
  next.layout_css_custom = String(next.layout_css_custom || "").trim().slice(0, 20000);
  next.layout_textos_custom = normalizeLayoutTextMap(next.layout_textos_custom);
  return next;
}
...
function fillConfigForm(config) {
  ...
  el.cfgLayoutCss.value = config.layout_css_custom || "";
  el.cfgLayoutTextos.value = JSON.stringify(config.layout_textos_custom || {}, null, 2);
...
function parseLayoutTextosInput(value) {
  const raw = String(value || "").trim();
  if (!raw) return {};
  let parsed = null;
  try {
    ...
    throw new Error(
      "Textos customizados: JSON invalido. Use formato {\"chave\":\"valor\"}."
    );
  }
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
    throw new Error("Textos customizados: informe um objeto JSON.");
  }
  return normalizeLayoutTextMap(parsed);
}
...
function collectConfigPayloadFromForm() {
  ...
  return {
    ...
    layout_css_custom: String(el.cfgLayoutCss.value || "").trim().slice(0, 20000),
    layout_textos_custom: parseLayoutTextosInput(el.cfgLayoutTextos.value),
  };
}
...
async function adicionarMobileMaisPedidoCustom(event) {
  ...
  if (!produto) throw new Error("Produto selecionado não encontrado.");
...
function renderProdutos() {
  ...
  state.produtos.forEach((produto) => {
    ...
    const estoqueTxt = controlaEstoque ? String(produto.estoque_atual) : "Nao controlado";
    const estoqueDisableAttr = controlaEstoque ? "" : "disabled title=\"Sem controle de estoque\"";
    const row = document.createElement("div");
    ...
    row.innerHTML = `
      ...
      <div class="produto-controls">
        <span class="produto-stock">Estoque: <strong>${estoqueTxt}</strong></span>
        <span class="produto-tag ${controlaEstoque ? "" : "muted"}">${controlaEstoque ? "Controle de estoque" : "Sem controle de estoque"}</span>
        <div class="payment-actions">
          <button type="button" data-produto-edit="${produto.id}">Editar</button>
          <button type="button" data-produto-delta="${produto.id}" data-delta="-10" ${estoqueDisableAttr}>-10</button>
          <button type="button" data-produto-delta="${produto.id}" data-delta="-1" ${estoqueDisableAttr}>-1</button>
          <button type="button" data-produto-delta="${produto.id}" data-delta="1" ${estoqueDisableAttr}>+1</button>
          <button type="button" data-produto-delta="${produto.id}" data-delta="10" ${estoqueDisableAttr}>+10</button>
...
function comandaPermiteEdicaoItens(comanda) {
  if (!comanda) return false;
  return COMANDA_ITEM_EDIT_STATUSES.has(String(comanda.status || ""));
}

function getCatalogoComandaQuantidade(produtoId) {
  const atual = Number(state.catalogoComandaQtd[produtoId]);
  if (!Number.isInteger(atual) || atual < 1) return 1;
  return Math.min(999, atual);
}

function setCatalogoComandaQuantidade(produtoId, quantidade) {
  const normalizado = Number.parseInt(String(quantidade || "1"), 10);
  const valor = Number.isInteger(normalizado) && normalizado > 0
    ? Math.min(999, normalizado)
    : 1;
  state.catalogoComandaQtd[produtoId] = valor;
  return valor;
}

function renderCatalogoComanda() {
  if (!el.caixaCatalogoItens || !el.caixaMobileAtalhos) return;
  el.caixaMobileAtalhos.innerHTML = "";
  el.caixaCatalogoItens.innerHTML = "";

  const comanda = state.selecionada;
  if (!comanda) {
    const empty = document.createElement("p");
    empty.className = "small";
    empty.textContent = "Selecione uma comanda para adicionar itens.";
    el.caixaCatalogoItens.appendChild(empty);
    return;
  }

  const podeEditar = comandaPermiteEdicaoItens(comanda);
  const disabledAttr = podeEditar ? "" : "disabled";
  const produtosAtivos = state.produtos
    .filter((produto) => produto.ativo)
    .sort((a, b) => String(a.nome || "").localeCompare(String(b.nome || ""), "pt-BR"));
  const termo = String(el.caixaItemSearch ? el.caixaItemSearch.value : "").trim().toLowerCase();
  ...
  const atalhos = (state.mobileMaisPedidosCustom || [])
    .map((row) => {
      const produto = state.produtos.find((p) => p.id === Number(row.produto_id));
      if (!produto || !produto.ativo) return null;
      return {
        produto_id: produto.id,
        nome_exibicao: row.nome_exibicao || produto.nome,
      };
    })
    .filter(Boolean);
  ...
  if (atalhos.length) {
    ...
    title.textContent = "Atalhos do mobile:";
    el.caixaMobileAtalhos.appendChild(title);
    const row = document.createElement("div");
    row.className = "caixa-atalhos-row";
    row.innerHTML = atalhos.map((atalho) => (
      `<button type="button" class="btn-no" data-caixa-atalho-produto="${atalho.produto_id}" ${disabledAttr}>${atalho.nome_exibicao}</button>`
    )).join("");
    el.caixaMobileAtalhos.appendChild(row);
  }

  if (!produtosFiltrados.length) {
    const empty = document.createElement("p");
    empty.className = "small";
    empty.textContent = "Nenhum item no catalogo para esse filtro.";
    el.caixaCatalogoItens.appendChild(empty);
    return;
  }

  produtosFiltrados.forEach((produto) => {
    const qtd = getCatalogoComandaQuantidade(produto.id);
    const card = document.createElement("article");
    card.className = "caixa-item-card";
    card.innerHTML = `
      <img src="${productImageSrc(produto.imagem_url)}" alt="${produto.nome}">
      <strong>${produto.nome}</strong>
      <span class="small">${produto.categoria || "Sem categoria"} | R$ ${fmtMoney(produto.preco)}</span>
      <div class="caixa-item-steps">
        <button type="button" data-caixa-card-add1="${produto.id}" ${disabledAttr}>(+1)</button>
        <button type="button" data-caixa-card-minus="${produto.id}" ${disabledAttr}>-</button>
        <input type="text" inputmode="numeric" pattern="[0-9]*" value="${qtd}" data-caixa-card-qtd="${produto.id}" ${disabledAttr}>
        <button type="button" data-caixa-card-plus="${produto.id}" ${disabledAttr}>+</button>
        <button type="button" data-caixa-card-add="${produto.id}" ${disabledAttr}>Adicionar</button>
      </div>
    `;
    el.caixaCatalogoItens.appendChild(card);
  });
}

function renderComandaItensCaixa() {
  if (!el.caixaComandaItensList) return;
  el.caixaComandaItensList.innerHTML = "";

  if (!state.selecionada) {
    const empty = document.createElement("p");
    empty.className = "small";
    empty.textContent = "Selecione uma comanda para gerenciar os itens.";
    el.caixaComandaItensList.appendChild(empty);
    return;
  }

  const itens = Array.isArray(state.selecionada.itens) ? state.selecionada.itens : [];
  if (!itens.length) {
    const empty = document.createElement("p");
    empty.className = "small";
    empty.textContent = "Comanda sem itens.";
  ...
  const podeEditar = comandaPermiteEdicaoItens(state.selecionada);
  itens.forEach((item) => {
    const adicionais = Array.isArray(item.adicionais) ? item.adicionais : [];
    const adicionaisTxt = adicionais.length
      ? adicionais.map((ad) => `${ad.nome} x${ad.quantidade}`).join(", ")
      : "Sem adicionais";
    const row = document.createElement("div");
    row.className = "row";
    row.innerHTML = `
      ...
      <div class="small">${adicionaisTxt}</div>
      ${item.observacoes ? `<div class="small">Obs: ${item.observacoes}</div>` : ""}
      <div class="payment-actions">
        <button type="button" class="btn-no" data-caixa-remove-item="${item.id}" ${podeEditar ? "" : "disabled"}>
          Remover item
        </button>
      </div>
    `;
    el.caixaComandaItensList.appendChild(row);
  });
}

async function adicionarItemNaComandaSelecionada(produtoId, quantidade = 1) {
  if (!state.selecionada) throw new Error("Selecione uma comanda.");
  if (!comandaPermiteEdicaoItens(state.selecionada)) {
    throw new Error("Status atual nao permite adicionar itens.");
  }
  const produtoIdNum = Number(produtoId);
  if (!Number.isInteger(produtoIdNum) || produtoIdNum <= 0) {
    throw new Error("Produto invalido para adicionar.");
  }
  const qtd = setCatalogoComandaQuantidade(produtoIdNum, quantidade);
  ...
  await loadComandas();
  showToast(`Item adicionado: ${qtd} unidade(s).`);
}

async function removerItemDaComandaSelecionada(itemId) {
  if (!state.selecionada) throw new Error("Selecione uma comanda.");
  if (!comandaPermiteEdicaoItens(state.selecionada)) {
    throw new Error("Status atual nao permite remover itens.");
  }
  const itemIdNum = Number(itemId);
  if (!Number.isInteger(itemIdNum) || itemIdNum <= 0) {
    throw new Error("Item invalido para remocao.");
  }
  const confirmar = window.confirm("Remover este item da comanda selecionada?");
  if (!confirmar) return;

  await api(
    `/comandas/${state.selecionada.id}/itens/${itemIdNum}/forcar?repor_estoque=true`,
    { method: "DELETE" }
  );
  await selecionarComanda(state.selecionada.id);
  await loadComandas();
...
function renderAjustesItensPagamento() {
  ...
  itens.forEach((item) => {
    ...
    row.innerHTML = `
      ...
      <div class="payment-actions">
        <button type="button" data-force-remove-item="${item.id}" data-restock="true">
          Retirar e repor estoque
        </button>
        <button type="button" class="btn-no" data-force-remove-item="${item.id}" data-restock="false">
...
function resetProdutoForm() {
  ...
  if (el.produtoControlaEstoque) {
    el.produtoControlaEstoque.value = "true";
  }
...
function montarPayloadProduto(imagemUrlFinal = null) {
  ...
  const controlaEstoque = !el.produtoControlaEstoque || el.produtoControlaEstoque.value !== "false";
...
function preencherFormProdutoParaEdicao(produtoId) {
  ...
  if (el.produtoControlaEstoque) {
    el.produtoControlaEstoque.value = produto.controla_estoque === false ? "false" : "true";
  }
...
async function excluirAdicional(adicionalId) {
  ...
  const confirmar = window.confirm(
    `Excluir definitivamente '${adicional.nome}'? Ele será removido de todas as notas/comandas.`
  );
  if (!confirmar) return;
  ...
  await loadComandas();
  await loadAdicionais();
  showToast("Adicional excluído definitivamente.");
}
...
async function excluirProdutoDefinitivo(produtoId) {
  ...
  const confirmou = window.confirm(
    `Excluir definitivamente '${produto.nome}'? Ele será removido de todas as notas/comandas.`
  );
  if (!confirmou) return;
...
async function resetarComandasAtivas() {
  ...
  showToast(
    `Reset concluído: ${payload.comandas_resetadas} comandas e ${payload.codigos_liberados} codigos liberados.`
  );
}
...
function bind() {
  ...
  on(el.caixaItemSearch, "input", () => renderCatalogoComanda());
  on(el.caixaMobileAtalhos, "click", (event) => {
    const btn = event.target.closest("button[data-caixa-atalho-produto]");
    if (!btn) return;
    run(() => adicionarItemNaComandaSelecionada(Number(btn.dataset.caixaAtalhoProduto), 1));
  });
  on(el.caixaCatalogoItens, "input", (event) => {
    const input = event.target.closest("input[data-caixa-card-qtd]");
    if (!input) return;
    const produtoId = Number(input.dataset.caixaCardQtd || "0");
    const valor = setCatalogoComandaQuantidade(produtoId, input.value);
    input.value = String(valor);
  });
  on(el.caixaCatalogoItens, "click", (event) => {
    const add1 = event.target.closest("button[data-caixa-card-add1]");
    if (add1) {
      run(() => adicionarItemNaComandaSelecionada(Number(add1.dataset.caixaCardAdd1), 1));
      return;
    }
    const minus = event.target.closest("button[data-caixa-card-minus]");
    if (minus) {
      const produtoId = Number(minus.dataset.caixaCardMinus || "0");
      const atual = getCatalogoComandaQuantidade(produtoId);
      setCatalogoComandaQuantidade(produtoId, Math.max(1, atual - 1));
      renderCatalogoComanda();
      return;
    }
    const plus = event.target.closest("button[data-caixa-card-plus]");
    if (plus) {
      const produtoId = Number(plus.dataset.caixaCardPlus || "0");
      const atual = getCatalogoComandaQuantidade(produtoId);
      setCatalogoComandaQuantidade(produtoId, atual + 1);
      renderCatalogoComanda();
      return;
    }
    const add = event.target.closest("button[data-caixa-card-add]");
    if (add) {
      const produtoId = Number(add.dataset.caixaCardAdd || "0");
      const qtd = getCatalogoComandaQuantidade(produtoId);
      run(() => adicionarItemNaComandaSelecionada(produtoId, qtd));
    }
  });
  on(el.caixaComandaItensList, "click", (event) => {
    const removeBtn = event.target.closest("button[data-caixa-remove-item]");
    if (!removeBtn) return;
    run(() => removerItemDaComandaSelecionada(Number(removeBtn.dataset.caixaRemoveItem)));
  });
