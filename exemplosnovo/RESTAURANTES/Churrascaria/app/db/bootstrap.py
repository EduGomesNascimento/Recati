...
def ensure_schema(engine: Engine) -> None:
    ...
    if "pedidos" in tables:
        ...
        with engine.begin() as conn:
            # Migra fluxo legado para o novo fluxo ABERTO -> FINALIZADA.
            conn.execute(
                text(
                    "UPDATE pedidos "
                    "SET status = 'ABERTO' "
            ...
            conn.execute(
                text(
                    ...
                    "WHERE status NOT IN ('ABERTO', 'FINALIZADA', 'CANCELADO')"
                )
            )
    ...
    if "produtos" in tables:
        ...
        if "controla_estoque" not in product_columns:
            with engine.begin() as conn:
                conn.execute(
                    text(
                        "ALTER TABLE produtos "
                        "ADD COLUMN controla_estoque BOOLEAN NOT NULL DEFAULT 1"
                    )
                )
        with engine.begin() as conn:
            conn.execute(
                text(
                    "UPDATE produtos "
                    "SET controla_estoque = 0 "
                    "WHERE LOWER(COALESCE(nome, '')) LIKE '%espeto%'"
                )
            )
    ...
    if "comanda_codigos" in tables:
        ...
        with engine.begin() as conn:
            ...
            conn.execute(
                text(
                    "UPDATE comanda_codigos "
                    "SET status_visual = 'ABERTO' "
                    "WHERE status_visual IN ('EM_PREPARO', 'PRONTO')"
                )
            )
            conn.execute(
                text(
                    "UPDATE comanda_codigos "
                    "SET status_visual = 'FINALIZADA' "
                    "WHERE status_visual = 'ENTREGUE'"
                )
            )
            conn.execute(
                text(
                    "UPDATE comanda_codigos "
                    "SET status_visual = 'LIBERADO' "
                    "WHERE status_visual NOT IN ('LIBERADO', 'ABERTO', 'FINALIZADA', 'CANCELADO')"
                )
            )
            total_codigos = conn.execute(text("SELECT COUNT(*) FROM comanda_codigos")).scalar_one()
            if int(total_codigos or 0) == 0:
                # Banco novo: cria codigos padrao para o painel nao iniciar vazio.
                for numero in range(1, 21):
                    conn.execute(
                        text(
                            "INSERT INTO comanda_codigos (codigo, ativo, em_uso, status_visual) "
                            "VALUES (:codigo, 1, 0, 'LIBERADO')"
                        ),
