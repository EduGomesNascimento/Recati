...
def list_painel_comandas(db: Session, ativo: bool = True) -> list[dict]:
    ...
    # durante a montagem do painel quando status_visual esta em aberto.
    status_ativos = {
        StatusPedido.ABERTO.value,
    }
...
def liberar_codigo(db: Session, codigo_id: int, confirmar: bool = False) -> ComandaCodigo:
    ...
    if code.status_visual in {StatusPedido.FINALIZADA.value, StatusPedido.CANCELADO.value} and not confirmar:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Confirmacao obrigatoria para liberar comanda finalizada/cancelada.",
        )
    code.status_visual = STATUS_VISUAL_LIBERADO
    db.commit()
    db.refresh(code)
...
def abrir_comanda(db: Session, payload: ComandaAbrirIn) -> dict:
    ...
    pedido = Pedido(
        ...
        mesa=_normalize_optional_text(payload.mesa),
...
def list_historico(
    ...
    if status_filter is not None:
        ...
        stmt = stmt.where(Pedido.status.in_([StatusPedido.FINALIZADA, StatusPedido.CANCELADO]))
...
def resetar_comandas_ativas(db: Session) -> dict:
    ...
    for pedido in pedidos:
        ...
        if pedido.status == StatusPedido.FINALIZADA:
    ...
    # todo codigo ativo volta para uso livre, inclusive casos orfaos.
    codigos_ativos = list(
        db.scalars(
            select(ComandaCodigo).where(
                ComandaCodigo.ativo.is_(True),
            )
        ).all()
    )
...
def change_status(
    ...
    if (
        ...
        and pedido.status == StatusPedido.FINALIZADA
        and repor_estoque
    ):
    ...
    if codigo_entity:
        ...
        codigo_entity.em_uso = new_status not in {StatusPedido.FINALIZADA, StatusPedido.CANCELADO}
    elif new_status in {StatusPedido.FINALIZADA, StatusPedido.CANCELADO}:
...
def _ensure_editable_order(pedido: Pedido) -> None:
    if pedido.status not in ITEM_EDITABLE_STATUSES:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=(
                "Somente comandas em ABERTO ou FINALIZADA "
...
def _decrement_stock_for_product(db: Session, produto_id: int, quantidade: int) -> None:
    produto = db.get(Produto, produto_id)
    if not produto:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
    ...
    if produto.estoque_atual < quantidade:
        raise HTTPException(
            ...
            detail=(
                ...
                f"Disponível: {produto.estoque_atual}, necessário: {quantidade}."
            ),
        )
    produto.estoque_atual -= quantidade


def _increment_stock_for_product(db: Session, produto_id: int, quantidade: int) -> None:
    produto = db.get(Produto, produto_id)
    if not produto:
        return
    if not produto.controla_estoque:
    ...
    for produto_id, quantity in required.items():
        ...
        if produto.estoque_atual < quantity:
            raise HTTPException(
                ...
                detail=(
                    f"Estoque insuficiente para '{produto.nome}'. "
                    f"Disponível: {produto.estoque_atual}, necessário: {quantity}."
                ),
            )
