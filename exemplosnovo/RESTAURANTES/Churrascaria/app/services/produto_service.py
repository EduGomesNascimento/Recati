...
def patch_estoque(db: Session, produto_id: int, delta: int) -> Produto:
    ...
    if not produto.controla_estoque:
        raise HTTPException(
            ...
            detail="Produto sem controle de estoque nao permite ajuste manual.",
        )
...
def _normalize_produto_data(data: dict) -> dict:
    ...
    normalized["controla_estoque"] = bool(normalized.get("controla_estoque", True))
    if not normalized["controla_estoque"]:
        normalized["estoque_atual"] = int(normalized.get("estoque_atual") or 0)
    return normalized
...
def _set_produto_adicionais(db: Session, produto: Produto, adicional_ids: list[int]) -> None:
    ...
    current_ids = sorted(
        {
            int(link.adicional_id)
            for link in (produto.adicionais_links or [])
            if int(link.adicional_id) > 0
        }
    )
    ...
    # Evita colisao de UNIQUE(produto_id, adicional_id) no mesmo flush.
    produto.adicionais_links = []
    db.flush()
