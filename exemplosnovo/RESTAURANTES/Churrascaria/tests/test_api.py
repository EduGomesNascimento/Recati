...
def test_mobile_and_caixa_textos_e_rodape(client: TestClient):
    ...
    assert "Powered by ChurrascariaERP" in mobile.text

    assert ("C\u00f3digos de Comanda" in caixa.text) or ("CÃ³digos de Comanda" in caixa.text)
    assert ("A\u00e7\u00f5es da comanda" in caixa.text) or ("AÃ§Ãµes da comanda" in caixa.text)
    assert ("Salvar configura\u00e7\u00f5es" in caixa.text) or ("Salvar configuraÃ§Ãµes" in caixa.text)
    assert "Powered by ChurrascariaERP" in caixa.text
...
def test_produto_exclusao_definitiva_bloqueada_quando_ja_usado(client: TestClient):
    ...
    assert "não pode ser excluído" in delete_resp.json()["detail"]
...
def test_adicional_hard_delete_bloqueado_se_ja_usado(client: TestClient):
    ...
    hard_delete = client.delete(f"/adicionais/{adicional['id']}?hard=true")
    assert hard_delete.status_code == 400
    assert "não pode ser excluído definitivamente" in hard_delete.json()["detail"]
...
def test_faturamento_periodo(client: TestClient):
    ...
    assert "ChurrascariaERP" in relatorio.text
...
def test_erp_config_get_patch_reset(client: TestClient):
    ...
    assert current["empresa_nome"] == "ChurrascariaERP"
    ...
    patch_resp = client.patch(
        "/config/erp",
        json={
            "empresa_nome": "Churrascaria Teste",
    ...
    assert patched["empresa_nome"] == "Churrascaria Teste"
    ...
    assert reset_payload["empresa_nome"] == "ChurrascariaERP"
...
def test_textos_js_acentuados_essenciais():
    ...
    assert "Adicional excluído." in caixa_js
...
def test_cupom_cozinha_mostra_pedido_comanda_e_flag_alteracao(client: TestClient):
    ...
    assert "ALTERACAO" in cupom.text
