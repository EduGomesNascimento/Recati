...
def test_app_sem_mojibake_geral():
    # Detecta padrões típicos de texto UTF-8 quebrado (ex.: "minÃºsculo").
    padrao_mojibake = re.compile(r"(?:Ã[\x80-\xBF]|Â[\x80-\xBF]|ðŸ|\ufffd)")
    ...
    assert not encontrados, f"Arquivos com possível mojibake: {encontrados}"
